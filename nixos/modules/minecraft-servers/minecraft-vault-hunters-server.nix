{
  ...
  # itzg:minecraft-server will copy these mods from `/mods/*.jar` to `<SERVER-DATA-DIR>/mods/`
  EXTRA_MODS = let
  ...
  in
    pkgs.stdenvNoCC.mkDerivation {
      pname = "${SERVER_NAME_SLUG}-extra-mods";
      version = "1.0.0";
      srcs = [
        chunky
      ];
      ...
      installPhase = ''
        mkdir -p $out
        # Symbolic links doesn't work, itzg:minecraft-server uses `mcopy` which explicitly expects real files
        cp ${chunky} $out/${chunky.name}
      '';
    };
      ...
      containers = {
        "${SERVER_NAME_SLUG}" = {
          ...
          environment = rec {
            ...
            CF_DOWNLOADS_REPO = "/extras/downloaded-mods";
          ...
          volumes = [
            ...
            "${MANUALLY_DOWNLOADED_MODS}:/extras/downloaded-mods:ro"
            "${EXTRA_MODS}:/mods:ro"
          ];
          ports = [
