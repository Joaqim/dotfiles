# Taken from [1]:
# [1]: https://github.com/ambroisie/nix-config/blob/main/lib/modules.nix
{
  self,
  lib,
  ...
...
let
  ...
  implOptionalRecursion =
    ...
    let
      ...
      impl =
        dir: fn:
        mapFilterAttrs (n: _: n != "" && !(hasPrefix "_" n)) (
          n: v:
          let
            path = "${toString dir}/${n}";
          in
          if v == "directory" then
            if pathExists "${path}/default.nix" then nameValuePair n (fn path) else recurseStep n path fn
